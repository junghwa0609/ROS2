...
class SafetyNode(Node):
    ...
    def scan_callback(self, scan_msg):
        ...
        # 2. create mask
        front_angle_min = -np.pi/12
        front_angle_max = np.pi/12
        front_mask = (angles >= front_angel_min) & (angles<= front_anlge_max)
        
        # 3. get only the front one
        front_ranges = ranges[front_mask]

        # 무한대 (inf)나 NaN 제거
        valid_ranges = ranges[np.isfinite(front_ranges)]
